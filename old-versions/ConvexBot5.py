...
stats_cur_over_total = []
stats_turn_number = []
stats_turn_over_max = []
stats_number_of_ships = []
...
minDropoffBuildingHalite = 7000
minDistancefromShipyard = 4 #8
grid_size = 4
depletion_limit = 2225
reqHaliteInGridForBuildingDropoff = 10000
...
while True:
    ...
    # Statistics
    stats_number_of_ships.append(len(me.get_ships()) / 10)
    ...
    for ship in me.get_ships():
        ...
        # What is each ship doing?
        # logging.info(f'id: {ship.id} state: {ship_states[ship.id]} return point: {ship_returnpoint[ship.id]} cast point: {ship_castpoint[ship.id]}')
    ...
    #Do we need to spawn a ship?
        #Stats
    stats_cur_over_total.append(me.halite_amount / (me.halite_amount + halite_spent))
    stats_turn_number.append(game.turn_number)
    stats_turn_over_max.append(round((game.turn_number) / constants.MAX_TURNS, 2))
    # logging.info(f'C/T: {round(stats_cur_over_total[-1], 2)} T/M: {round((game.turn_number / constants.MAX_TURNS), 2)}')
    ...
    #     square_shit = []
    #     for x in stats_turn_over_max:
    #         square_shit.append((5*math.pow(20, x))/100)
    #     plt.plot(stats_turn_number, square_shit)
    #     plt.plot(stats_turn_number, stats_cur_over_total)
    #     plt.plot(stats_turn_number, stats_number_of_ships)
    #     plt.xlabel('Turn Number')
    #     plt.ylabel('Current Halite / Total Halite')
    #     plt.title('Halite')
    #     plt.show()